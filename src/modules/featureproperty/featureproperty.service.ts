...
export class FeaturePropertyService {
  ...
  async findOne(id: number): Promise<FeaturePropertyEntity> {
    const featurepropertyData = await this.featurepropertyRepository.findOneBy({ id });
    if (!featurepropertyData) {
      throw new HttpException('Featureproperty Not Found', 404);
    }
  ...
  async update(id: number, updateFeaturepropertyDto: UpdateFeaturepropertyDto) {
    ...
    const featurepropertyData = this.featurepropertyRepository.merge(existingFeatureproperty, updateFeaturepropertyDto);
    return await this.featurepropertyRepository.save(featurepropertyData);
  }

  async remove(id: number) {
    const existingFeatureproperty = await this.findOne(id);
    return await this.featurepropertyRepository.remove(existingFeatureproperty);
  }
}
