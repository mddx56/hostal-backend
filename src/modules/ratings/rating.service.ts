...
export class RatingService {
  ...
  async findOne(id: number): Promise<RatingEntity> {
    const ratingData = await this.ratingRepository.findOneBy({ id });
    if (!ratingData) {
  ...
  async update(id: number, updateRatingDto: UpdateRatingDto) {
    ...
    const ratingData = this.ratingRepository.merge(existingRating, updateRatingDto);
    return await this.ratingRepository.save(ratingData);
  }

  async remove(id: number) {
    const existingRating = await this.findOne(id);
    return await this.ratingRepository.remove(existingRating);
  }
}
