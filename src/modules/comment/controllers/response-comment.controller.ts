...
export class RescommentController {
  ...
  async create(@Body() createRescommentDto: CreateResponseCommentDto) {
    try {
      await this.rescommentService.create(createRescommentDto);
  ...
  async findAll() {
    try {
      const data = await this.rescommentService.findAll();
      return {
        success: true,
        data,
  ...
  async findOne(@Param('id') id: string) {
    try {
      const data = await this.rescommentService.findOne(+id);
      return {
        success: true,
        data,
        message: 'Rescomment Fetched Successfully',
  ...
  async update(@Param('id') id: string, @Body() updateRescommentDto: UpdateResponseCommentDto) {
    try {
      await this.rescommentService.update(+id, updateRescommentDto);
      return {
        success: true,
